...
const email = `reading-comprehension-student-${crypto.randomUUID()}@example.test`
...
function readingTopic(curriculum) { return curriculum.subjects.find((subject) => subject.slug === 'verbal-ability')?.topics.find((topic) => topic.slug === 'reading-comprehension') }

await api('/api/auth/register', { method: 'POST', body: JSON.stringify({ email, password, firstName: 'Reading', lastName: 'Student' }) })
...
let topic = readingTopic(await api('/api/student/courses/cse-professional/curriculum'))
assert(topic?.position === 10 && topic.lessons.length === 12, 'Reading Comprehension topic ordering or lesson count is invalid.')
...
topic = readingTopic(await api('/api/student/courses/cse-professional/curriculum'))
...
assert(generated.questions.every((question) => question.prompt.includes('Passage:') && question.prompt.includes('\n\nQuestion:')), 'Generated passage formatting data is missing.')
...
for (const question of generated.questions) {
  const expected = /Clearer Service Queue/iu.test(question.prompt) ? 'Numbered sections and a display make the service queue clearer for visitors and clerks.' : /Shared Digital Records/iu.test(question.prompt) ? 'A controlled shared record reduces duplicate work and keeps attendance information consistent.' : /Planning a Community Garden/iu.test(question.prompt) ? 'Observation, organized duties, and records helped residents adapt a garden to its site.' : null
...
assert(generatedResult.questions.every((question) => question.explanation?.length > 0 && question.prompt.includes('Passage:')), 'Generated review lacks passages or explanations.')

localSql(`UPDATE lesson_progress SET status='completed',completed_at=CURRENT_TIMESTAMP,last_viewed_at=CURRENT_TIMESTAMP,progress_percent=100 WHERE user_id=(SELECT id FROM users WHERE email='${email}') AND lesson_id IN (SELECT l.id FROM lessons l JOIN topics t ON t.id=l.topic_id WHERE t.slug='reading-comprehension' AND l.position<=10); INSERT OR IGNORE INTO lesson_progress (user_id,lesson_id,status,started_at,completed_at,last_viewed_at,progress_percent) SELECT u.id,l.id,'completed',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,100 FROM users u CROSS JOIN lessons l JOIN topics t ON t.id=l.topic_id WHERE u.email='${email}' AND t.slug='reading-comprehension' AND l.position<=10;`)
topic = readingTopic(await api('/api/student/courses/cse-professional/curriculum'))
...
console.log('STUDENT_FLOW_OK topic_position=10 lessons=12 intro_blocks=12 generated_questions=5 passage_format=preserved generated_refresh=stable generated_score=100 fixed_questions=8 fixed_score=100 quiz_questions=15 quiz_score=100 explanations=visible sequential_unlocking=verified')
