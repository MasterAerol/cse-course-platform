...
import { blocksFor, fixedQuestion, generatedByLesson, lessonSpecs, mixedQuestions, quizQuestions, subjectSlug, topicDescription, topicSlug, validateQuestions } from '../scripts/subject-verb-agreement-topic-content.mjs'
...
describe('Subject–Verb Agreement publisher content', () => {
  it('defines the sixth Verbal topic and exactly twelve ordered lessons', () => {
    expect(subjectSlug).toBe('verbal-ability')
    expect(topicSlug).toBe('subject-verb-agreement')
    expect(topicDescription).toBe('A structured course on matching subjects and verbs in number and person, including compound subjects, indefinite pronouns, collective nouns, intervening phrases, inverted sentences, and special agreement cases.')
  ...
  it('provides mobile-safe instructional blocks and nine registered practices', () => {
    ...
    expect(blocksFor('understanding-subject-verb-agreement')).toHaveLength(12)
