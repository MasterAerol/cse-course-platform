...
import { fractionsLessonSpecs } from './lib/fractions-teaching-system-content.mjs'

const confirmation = 'publish-fractions-teaching-system-v1'
...
async function main() {
  ...
  if (cookie === null) {
    ...
    if (email === undefined || password === undefined) throw new Error('Pass --cookie, or --email with --password or CSE_FRACTIONS_ADMIN_PASSWORD.')
  ...
  const topic = subject?.topics.find((item) => item.slug === 'fractions')
  if (topic === undefined || topic.lessons.length !== fractionsLessonSpecs.length) throw new Error('Fractions must contain exactly the twelve expected lessons.')

  const states = []
  for (const [lessonIndex, spec] of fractionsLessonSpecs.entries()) {
    const lesson = topic.lessons.find((item) => item.slug === spec.slug)
    if (lesson === undefined) throw new Error(`Missing Fractions lesson ${spec.slug}.`)
  ...
  if (validateOnly) {
    console.log(JSON.stringify({ valid: true, operation: 'fractions-teaching-system-v1', topicSlug: 'fractions', lessonCount: fractionsLessonSpecs.length, writesRequired: totals.lessonsChanged > 0, totals, lessons: reports }, null, 2))
    return
  }

  for (const state of states) {
    if (!state.plan.writesRequired) continue
    await request(`/api/admin/lessons/${state.lesson.id}/fractions-teaching-system-v1`, {
      method: 'PUT',
  ...
  console.log(JSON.stringify({ published: true, updated: totals.lessonsChanged > 0, operation: 'fractions-teaching-system-v1', topicSlug: 'fractions', lessonCount: fractionsLessonSpecs.length, totals, lessons: verifiedLessons, unrelatedTopicsModified: 0 }, null, 2))
}
