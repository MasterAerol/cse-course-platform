...
adminLessonBlockRoutes.put(
  '/lessons/:lessonId/simple-interest-teaching-system-v1',
  async (context) => {
    const params = parseValidatedInput(
      lessonIdParamsSchema.safeParse({ lessonId: context.req.param('lessonId') }),
    )
    const input = await parseJsonBody(context, simpleInterestTeachingSystemReconcileSchema)
    return successResponse(
      context,
...
adminLessonBlockRoutes.put(
  '/lessons/:lessonId/synonyms-antonyms-teaching-system-v1',
  async (context) => {
    const params = parseValidatedInput(
      lessonIdParamsSchema.safeParse({ lessonId: context.req.param('lessonId') }),
    )
    const input = await parseJsonBody(context, synonymsAntonymsTeachingSystemReconcileSchema)
    return successResponse(
      context,
...
adminLessonBlockRoutes.put(
  '/lessons/:lessonId/sentence-completion-teaching-system-v1',
  async (context) => {
    const params = parseValidatedInput(
      lessonIdParamsSchema.safeParse({ lessonId: context.req.param('lessonId') }),
    )
    const input = await parseJsonBody(context, sentenceCompletionTeachingSystemReconcileSchema)
    return successResponse(
      context,
...
adminLessonBlockRoutes.get(
  '/lessons/:lessonId/reading-comprehension-teaching-system-v1/capability',
  async (context) => {
    const params = parseValidatedInput(
      lessonIdParamsSchema.safeParse({ lessonId: context.req.param('lessonId') }),
    )
    return successResponse(
      context,
      await getReadingComprehensionTeachingSystemCapability(context.env.DB, params.lessonId),
    )
  },
)
adminLessonBlockRoutes.put(
  '/lessons/:lessonId/reading-comprehension-teaching-system-v1',
  async (context) => {
    const params = parseValidatedInput(
      lessonIdParamsSchema.safeParse({ lessonId: context.req.param('lessonId') }),
    )
    const input = await parseJsonBody(context, readingComprehensionTeachingSystemReconcileSchema)
    return successResponse(
      context,
