...
describe('Full CSE mock persistence and timing',()=>{
  ...
  beforeAll(async()=>{
    ...
    const mock=await env.DB.prepare("INSERT INTO mock_examinations(public_id,course_id,title,slug,description,simulation_label,position,passing_score,question_count,timed_duration_minutes,current_blueprint_version,status,source_url) VALUES('mock-api-exam',?1,'Mock API','mock-api-exam','test','Platform-Designed Subject Distribution v1',99,80,150,190,1,'draft','https://csc.gov.ph')").bind(course.id).run();const mockId=Number(mock.meta.last_row_id)
    const blueprint=await env.DB.prepare("INSERT INTO mock_exam_blueprints(mock_exam_id,version,label,total_questions,passing_score_percent,timed_duration_minutes,easy_count,medium_count,hard_count) VALUES(?1,1,'Platform-Designed Subject Distribution v1',150,80,190,45,75,30)").bind(mockId).run();const blueprintId=Number(blueprint.meta.last_row_id)
