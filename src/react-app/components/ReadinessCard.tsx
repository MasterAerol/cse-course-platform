...
export function ReadinessCardView({ readiness }: { readiness: CseReadiness }) {
  ...
  return (
    <section className="continue-card readiness-card" data-testid="readiness-card" aria-labelledby="readiness-card-title">
      <p className="eyebrow">CSE Readiness</p>
      <h3 className="readiness-card__score" id="readiness-card-title">
        {readiness.hasSufficientEvidence ? `${readiness.score} / 100` : 'Build your readiness estimate'}
      </h3>
      {readiness.hasSufficientEvidence && (
        <p className="readiness-card__metric-label">Readiness score</p>
      )}
      <p className="readiness-card__signal"><strong>{readinessBandLabel(readiness.readinessBand)}</strong> · {confidenceLabel(readiness.confidence)}</p>
      {strongest !== undefined && <p className="meta-copy">Strongest: {strongest.title}</p>}
      {weakest !== undefined && <p className="meta-copy">Needs attention: {weakest.title}</p>}
      <div className="readiness-card__actions">
        <Link className="button-link" to="/readiness">View readiness</Link>
        <Link className="button-link button-link--secondary" to={readiness.recommendation.route}>{readiness.recommendation.actionLabel}</Link>
      </div>
    </section>
  )
}
export function ReadinessCardContent({ state }: { state: ReadinessViewState }) {
  if (state.status === 'loading') return <section className="continue-card readiness-card" aria-live="polite" aria-busy="true"><PasaWiseLoader compact label="Calculating CSE Readiness…" /></section>
