...
import { vocabularyWordMeaningLessonSpecs } from './lib/vocabulary-word-meaning-teaching-system-content.mjs'

const confirmation = 'publish-vocabulary-word-meaning-teaching-system-v1'
...
function buildPlan(existingBlocks, desiredBlocks, lessonSlug) {
  ...
  const deletions = [...guided, ...excess].map((block) => ({
    ...
    reason: block.type === guidedBlockType ? 'IllustratedGuidedTeaching is outside the approved Vocabulary and Word Meaning v1 teaching architecture.' : 'The block is beyond the canonical lesson block sequence.',
  ...
  return {
    ...
    blocksDeleted: guided.length + excess.length,
    deletions,
    writesRequired: guided.length + excess.length + updates.length + creates.length > 0,
  }
}
...
async function main() {
  ...
  if (cookie === null) {
    ...
    const password = process.env.CSE_VOCABULARY_ADMIN_PASSWORD
    if (email === undefined || password === undefined) throw new Error('Pass --cookie, or --email with CSE_VOCABULARY_ADMIN_PASSWORD set securely in the environment.')
  ...
  const topic = subject?.topics.find((item) => item.slug === 'vocabulary-and-word-meaning')
  if (topic === undefined || topic.lessons.length !== vocabularyWordMeaningLessonSpecs.length) throw new Error('Vocabulary and Word Meaning must contain exactly the twelve expected lessons.')

  const states = []
  for (const [lessonIndex, spec] of vocabularyWordMeaningLessonSpecs.entries()) {
    const lesson = topic.lessons.find((item) => item.slug === spec.slug)
    if (lesson === undefined) throw new Error(`Missing Vocabulary and Word Meaning lesson ${spec.slug}.`)
  ...
  const reports = states.map(({ spec, lesson, existingBlocks, plan }) => ({
    ...
    currentBlockCount: existingBlocks.length,
    desiredBlockCount: spec.blocks.length,
    guidedBlocksRemoved: plan.guidedCount,
    blocksCreated: plan.blocksCreated,
    blocksUpdated: plan.blocksUpdated,
    blocksDeleted: plan.blocksDeleted,
  ...
  if (validateOnly) {
    console.log(JSON.stringify({ valid: true, operation: 'vocabulary-word-meaning-teaching-system-v1', topicSlug: 'vocabulary-and-word-meaning', lessonCount: vocabularyWordMeaningLessonSpecs.length, writesRequired: totals.lessonsChanged > 0, totals, deletionReviewRequired, deletionPlanFingerprint, deletions: deletionPlan, lessons: reports }, null, 2))
    return
  }

  if (deletionReviewRequired && args.get('approve-deletions') !== deletionPlanFingerprint) {
    throw new Error(`Review every planned deletion, then pass --approve-deletions ${deletionPlanFingerprint}.`)
  }
  for (const state of states) {
    if (!state.plan.writesRequired) continue
    await request(`/api/admin/lessons/${state.lesson.id}/vocabulary-word-meaning-teaching-system-v1`, {
      method: 'PUT',
  ...
  console.log(JSON.stringify({ published: true, updated: totals.lessonsChanged > 0, operation: 'vocabulary-word-meaning-teaching-system-v1', topicSlug: 'vocabulary-and-word-meaning', lessonCount: vocabularyWordMeaningLessonSpecs.length, totals, lessons: verifiedLessons, unrelatedTopicsModified: 0 }, null, 2))
}
