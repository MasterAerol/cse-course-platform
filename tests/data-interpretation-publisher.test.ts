...
import { blocksFor, fixedQuestion, generatedByLesson, lessonSpecs, mixedQuestions, quizQuestions, topicDescription, topicSlug, validateQuestions } from '../scripts/data-interpretation-topic-content.mjs'
...
describe('Data Interpretation publisher content', () => {
  it('defines the ninth Analytical Ability topic and twelve ordered lessons', () => {
    expect(topicSlug).toBe('data-interpretation')
    expect(topicDescription).toBe('A structured course on reading tables, bar charts, line graphs, pie charts, percentages, ratios, averages, comparisons, and multi-step data questions.')
  ...
  it('provides substantial safe teaching blocks before every practice', () => {
    ...
    const introduction = blocksFor('understanding-data-displays')
    expect(introduction).toHaveLength(14)
    expect(introduction.filter((block) => block.blockType === 'formula')).toHaveLength(4)
    expect(JSON.stringify(introduction)).toContain('Verify the answer against the display')
  })
