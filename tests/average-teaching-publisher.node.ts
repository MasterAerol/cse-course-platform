...
const script = path.join(root, 'scripts', 'create-and-publish-average-teaching-system.mjs')
const lessons = averageLessonSpecs.map((spec, index) => ({ ...spec, id: 101 + index, position: index + 1, status: 'published' as const }))
...
const productionLikeCounts = [10, 9, 8, 8, 8, 8, 8, 8, 8, 8, 8, 4]
...
for (const lesson of lessons) {
  ...
  if (lesson.slug === 'finding-a-missing-value') {
...
async function handleRequest(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  if (url === '/api/admin/courses/1') {
    send(response, { subjects: [{ slug: 'numerical-ability', topics: [{ slug: 'average', lessons: lessons.map(({ id, slug, title, lessonType, estimatedMinutes, status, position }) => ({ id, slug, title, lessonType, estimatedMinutes, status, position })) }] }] })
    return
  }

  const reconcileMatch = url.match(/^\/api\/admin\/lessons\/(\d+)\/average-teaching-system-v1$/u)
...
function runPublisher(extraArgs: string[] = []): Promise<{ stdout: string; stderr: string; status: number | null }> {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [script, '--base-url', baseUrl, '--cookie', 'test-admin-session', '--confirm', 'publish-average-teaching-system-v1', ...extraArgs], { cwd: root, windowsHide: true })
...
describe('Average Teaching System v1 publisher', () => {
  it('reconciles the production-shaped lesson structure, preserves a retained block ID, and is idempotent', async () => {
    ...
    expect(validationPlan).toMatchObject({
      ...
      totals: {
        lessonsChanged: 12,
        blocksCreated: 0,
        blocksUpdated: desiredBlockCount,
        blocksDeleted: 14,
    ...
    expect(validationPlan.deletions).toEqual(expect.arrayContaining([expect.objectContaining({ lessonSlug: 'understanding-average', position: averageLessonSpecs[0].blocks.length + 1, learnerContentAssessment: 'requires-human-review' })]))
