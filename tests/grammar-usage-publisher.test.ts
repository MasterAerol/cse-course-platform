...
import { blocksFor, fixedQuestion, generatedByLesson, lessonSpecs, mixedQuestions, quizQuestions, subjectSlug, topicDescription, topicSlug, validateQuestions } from '../scripts/grammar-usage-topic-content.mjs'
...
describe('Grammar and Correct Usage publisher content', () => {
  it('defines the fifth Verbal topic and exactly twelve ordered lessons', () => {
    expect(subjectSlug).toBe('verbal-ability')
    expect(topicSlug).toBe('grammar-and-correct-usage')
    expect(topicDescription).toBe('A structured course on standard English grammar, correct word forms, verb usage, modifiers, prepositions, conjunctions, and common usage errors.')
  ...
  it('provides mobile-safe instructional blocks and nine registered practices', () => {
    ...
    expect(blocksFor('understanding-standard-english-usage')).toHaveLength(12)
