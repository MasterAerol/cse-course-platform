...
const script=path.join(root,'scripts','create-and-publish-pronouns-modifiers-teaching-system.mjs')
const lessons=pronounsModifiersLessonSpecs.map((spec,index)=>({...spec,id:201+index,position:index+1,status:'published' as const}))
...
for(const [lessonIndex,lesson] of lessons.entries()){
  ...
  if(lesson.slug==='adjective-and-adverb-modifiers') preservedRetainedBlockId=blocks[4]?.id??0
...
describe('Pronouns and Modifiers Teaching System v1 publisher',()=>{
  it('validates read-only, publishes canonically, preserves IDs, proves idempotency, and fails closed on unknown deletion',async()=>{
    const validation=await runPublisher(['--validate-only']);expect(validation).toMatchObject({status:0,stderr:''});expect(JSON.parse(validation.stdout)).toMatchObject({valid:true,topicSlug:'pronouns-and-modifiers',lessonCount:12,writesRequired:true,deletionReviewRequired:false,deletionPlanFingerprint:null,totals:{lessonsChanged:12,blocksCreated:desiredBlockCount-productionBlockCount,blocksUpdated:productionBlockCount,blocksDeleted:0,guidedBlocksRemoved:0}});expect(mutationCalls).toBe(0);expect(capabilityCalls).toBeGreaterThan(0)
    const first=await runPublisher();expect(first).toMatchObject({status:0,stderr:''});expect(JSON.parse(first.stdout)).toMatchObject({published:true,updated:true,topicSlug:'pronouns-and-modifiers',lessonCount:12,totals:{lessonsChanged:12,blocksCreated:desiredBlockCount-productionBlockCount,blocksUpdated:productionBlockCount,blocksDeleted:0,guidedBlocksRemoved:0},unrelatedTopicsModified:0});expect(findBlock(preservedRetainedBlockId)).not.toBeNull()
    ...
    const second=await runPublisher();expect(second).toMatchObject({status:0,stderr:''});expect(JSON.parse(second.stdout)).toMatchObject({published:true,updated:false,topicSlug:'pronouns-and-modifiers',totals:{lessonsChanged:0,blocksCreated:0,blocksUpdated:0,blocksDeleted:0,guidedBlocksRemoved:0},unrelatedTopicsModified:0});expect(mutationCalls).toBe(mutationsAfterFirst)
