...
describe('Full CSE mock publisher contract',()=>{
  it('builds a credential-free draft/published payload accepted by the admin schema',()=>{
    ...
    expect(mockExamDistributionNotice).toBe('This mock uses a platform-designed subject distribution and is not an official CSC item allocation.')
  ...
  it('contains the exact versioned allocation and all reusable generators',()=>{
    expect(fullCseMockBlueprintV1).toMatchObject({version:1,label:'Platform-Designed Subject Distribution v1',totalQuestions:150,passingScorePercent:80,timedDurationMinutes:190,difficulty:{easy:45,medium:75,hard:30}})
