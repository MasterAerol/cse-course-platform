...
for (const lesson of lessons) {
  ...
  if (lesson.slug === 'adding-fractions') {
    ...
    blocks.push({ id: nextBlockId++, type: 'illustrated-guided-teaching', content: { title: 'Obsolete Fractions pilot' }, position: 5 })
...
describe('Fractions Teaching System v1 publisher', () => {
  it('validates all lessons, removes the guided pilot, preserves the visual board, and is idempotent', async () => {
    ...
    expect(JSON.parse(validation.stdout)).toMatchObject({ valid: true, lessonCount: 12, writesRequired: true, totals: { lessonsChanged: 12, guidedBlocksRemoved: 1 } })
    expect(mutationCalls).toBe(0)

    const first = await runPublisher()
    expect(first).toMatchObject({ status: 0, stderr: '' })
    expect(JSON.parse(first.stdout)).toMatchObject({ published: true, updated: true, lessonCount: 12, totals: { lessonsChanged: 12, guidedBlocksRemoved: 1 }, unrelatedTopicsModified: 0 })
