...
import { synonymsAntonymsLessonSpecs } from './lib/synonyms-antonyms-teaching-system-content.mjs'

const confirmation = 'publish-synonyms-antonyms-teaching-system-v1'
...
function buildPlan(existingBlocks, desiredBlocks, lessonSlug) {
  ...
  const deletions = [...guided, ...excess].map((block) => ({
    ...
    reason: block.type === guidedBlockType ? 'IllustratedGuidedTeaching is outside the approved Synonyms and Antonyms v1 teaching architecture.' : 'The block is beyond the canonical lesson block sequence.',
...
async function main() {
  ...
  if (cookie === null) {
    ...
    const password = process.env.CSE_SYNONYMS_ANTONYMS_ADMIN_PASSWORD
    if (email === undefined || password === undefined) throw new Error('Pass --cookie, or --email with CSE_SYNONYMS_ANTONYMS_ADMIN_PASSWORD set securely in the environment.')
  ...
  const topic = subject?.topics.find((item) => item.slug === 'synonyms-and-antonyms')
  if (topic === undefined || topic.lessons.length !== synonymsAntonymsLessonSpecs.length) throw new Error('Synonyms and Antonyms must contain exactly the twelve expected lessons.')

  const states = []
  for (const [lessonIndex, spec] of synonymsAntonymsLessonSpecs.entries()) {
    const lesson = topic.lessons.find((item) => item.slug === spec.slug)
    if (lesson === undefined) throw new Error(`Missing Synonyms and Antonyms lesson ${spec.slug}.`)
  ...
  for (const state of states) {
    ...
    await request(`/api/admin/lessons/${state.lesson.id}/synonyms-antonyms-teaching-system-v1`, {
      method: 'PUT',
  ...
  console.log(JSON.stringify({ published: true, updated: totals.lessonsChanged > 0, operation: 'synonyms-antonyms-teaching-system-v1', topicSlug: 'synonyms-and-antonyms', lessonCount: synonymsAntonymsLessonSpecs.length, totals, lessons: verifiedLessons, unrelatedTopicsModified: 0 }, null, 2))
}
