...
import { grammarCorrectUsageLessonSpecs } from './lib/grammar-correct-usage-teaching-system-content.mjs'

const confirmation = 'publish-grammar-correct-usage-teaching-system-v1'
...
function buildPlan(existingBlocks, desiredBlocks) {
  ...
  const deletions = [...guided, ...excess].map((block) => ({
    ...
    reason: block.type === guidedBlockType ? 'IllustratedGuidedTeaching is outside the approved Grammar and Correct Usage v1 teaching architecture.' : 'The block is beyond the canonical lesson block sequence.',
...
async function main() {
  ...
  if (cookie === null) {
    ...
    const password = process.env.CSE_GRAMMAR_USAGE_ADMIN_PASSWORD
    if (email === undefined || password === undefined) throw new Error('Pass --cookie, or --email with CSE_GRAMMAR_USAGE_ADMIN_PASSWORD set securely in the environment.')
  ...
  for (const [lessonIndex, spec] of grammarCorrectUsageLessonSpecs.entries()) {
    ...
    if (lesson === undefined) throw new Error(`Missing Grammar and Correct Usage lesson ${spec.slug}.`)
  ...
  if (validateOnly) {
    console.log(JSON.stringify({ valid: true, operation: 'grammar-correct-usage-teaching-system-v1', topicSlug: 'grammar-and-correct-usage', lessonCount: grammarCorrectUsageLessonSpecs.length, writesRequired: totals.lessonsChanged > 0, totals, deletionReviewRequired, deletionPlanFingerprint, deletions: deletionPlan, lessons: reports }, null, 2))
    return
  }
  ...
  for (const state of states) {
    ...
    await request(`/api/admin/lessons/${state.lesson.id}/grammar-correct-usage-teaching-system-v1`, { method: 'PUT', body: JSON.stringify({ blocks: state.spec.blocks.map((block, index) => desiredPayload(block, index + 1)) }) })
  }
  ...
  console.log(JSON.stringify({ published: true, updated: totals.lessonsChanged > 0, operation: 'grammar-correct-usage-teaching-system-v1', topicSlug: 'grammar-and-correct-usage', lessonCount: grammarCorrectUsageLessonSpecs.length, totals, lessons: verifiedLessons, unrelatedTopicsModified: 0 }, null, 2))
}
