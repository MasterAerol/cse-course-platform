...
import { blocksFor, fixedQuestion, generatedByLesson, lessonSpecs, mixedQuestions, quizQuestions, topicDescription, topicSlug, validateQuestions } from '../scripts/seating-arrangements-topic-content.mjs'
...
describe('Seating and Arrangement publisher content', () => {
  it('defines the eighth Analytical Ability topic and twelve ordered lessons', () => {
    expect(topicSlug).toBe('seating-and-arrangement-problems')
    expect(topicDescription).toBe('A structured course on arranging people or objects in rows, circles, positions, schedules, and simple logical layouts using exact clues.')
  ...
  it('provides substantial safe teaching blocks before every practice', () => {
    ...
    const introduction = blocksFor('understanding-arrangements-and-positions')
    expect(introduction).toHaveLength(14)
    expect(introduction.filter((block) => block.blockType === 'formula')).toHaveLength(4)
    expect(JSON.stringify(introduction)).toContain('Reject ambiguous or contradictory layouts')
  })
