...
import { sentenceCompletionLessonSpecs } from './lib/sentence-completion-teaching-system-content.mjs'

const confirmation = 'publish-sentence-completion-teaching-system-v1'
...
function buildPlan(existingBlocks, desiredBlocks, lessonSlug) {
  ...
  const deletions = [...guided, ...excess].map((block) => ({
    ...
    reason: block.type === guidedBlockType ? 'IllustratedGuidedTeaching is outside the approved Sentence Completion v1 teaching architecture.' : 'The block is beyond the canonical lesson block sequence.',
...
async function main() {
  ...
  if (cookie === null) {
    ...
    const password = process.env.CSE_SENTENCE_COMPLETION_ADMIN_PASSWORD
    if (email === undefined || password === undefined) throw new Error('Pass --cookie, or --email with CSE_SENTENCE_COMPLETION_ADMIN_PASSWORD set securely in the environment.')
  ...
  const topic = subject?.topics.find((item) => item.slug === 'sentence-completion')
  if (topic === undefined || topic.lessons.length !== sentenceCompletionLessonSpecs.length) throw new Error('Sentence Completion must contain exactly the twelve expected lessons.')

  const states = []
  for (const [lessonIndex, spec] of sentenceCompletionLessonSpecs.entries()) {
    const lesson = topic.lessons.find((item) => item.slug === spec.slug)
    if (lesson === undefined) throw new Error(`Missing Sentence Completion lesson ${spec.slug}.`)
  ...
  if (validateOnly) {
    console.log(JSON.stringify({ valid: true, operation: 'sentence-completion-teaching-system-v1', topicSlug: 'sentence-completion', lessonCount: sentenceCompletionLessonSpecs.length, writesRequired: totals.lessonsChanged > 0, totals, deletionReviewRequired, deletionPlanFingerprint, deletions: deletionPlan, lessons: reports }, null, 2))
    return
  }
  ...
  for (const state of states) {
    ...
    await request(`/api/admin/lessons/${state.lesson.id}/sentence-completion-teaching-system-v1`, {
      method: 'PUT',
  ...
  console.log(JSON.stringify({ published: true, updated: totals.lessonsChanged > 0, operation: 'sentence-completion-teaching-system-v1', topicSlug: 'sentence-completion', lessonCount: sentenceCompletionLessonSpecs.length, totals, lessons: verifiedLessons, unrelatedTopicsModified: 0 }, null, 2))
}
