...
const confirmation = 'publish-decimals-teaching-system-v1'
...
function buildPlan(existingBlocks, desiredBlocks) {
  ...
  const deletions = [...guided, ...excess].map((block) => ({
    ...
    reason: block.type === guidedBlockType ? 'IllustratedGuidedTeaching is outside the approved Decimals v1 teaching architecture.' : 'The block is beyond the canonical lesson block sequence.',
...
async function main() {
  ...
  if (cookie === null) {
    ...
    if (email === undefined || password === undefined) throw new Error('Pass --cookie, or --email with --password or CSE_DECIMALS_ADMIN_PASSWORD.')
  ...
  const topic = subject?.topics.find((item) => item.slug === 'decimals')
  if (topic === undefined || topic.lessons.length !== decimalsLessonSpecs.length) throw new Error('Decimals must contain exactly the twelve expected lessons.')

  const states = []
  for (const [lessonIndex, spec] of decimalsLessonSpecs.entries()) {
    const lesson = topic.lessons.find((item) => item.slug === spec.slug)
    if (lesson === undefined) throw new Error(`Missing Decimals lesson ${spec.slug}.`)
  ...
  if (validateOnly) {
    console.log(JSON.stringify({ valid: true, operation: 'decimals-teaching-system-v1', topicSlug: 'decimals', lessonCount: decimalsLessonSpecs.length, writesRequired: totals.lessonsChanged > 0, totals, deletionReviewRequired, deletionPlanFingerprint, deletions: deletionPlan, lessons: reports }, null, 2))
    return
  }
  ...
  for (const state of states) {
    ...
    await request(`/api/admin/lessons/${state.lesson.id}/decimals-teaching-system-v1`, {
      method: 'PUT',
  ...
  console.log(JSON.stringify({ published: true, updated: totals.lessonsChanged > 0, operation: 'decimals-teaching-system-v1', topicSlug: 'decimals', lessonCount: decimalsLessonSpecs.length, totals, lessons: verifiedLessons, unrelatedTopicsModified: 0 }, null, 2))
}
