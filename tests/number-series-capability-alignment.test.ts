...
import publisher from '../scripts/create-and-publish-number-series-teaching-system.mjs?raw'

describe('Number Series Teaching Release capability', () => {
  it('uses the deployed shared Analytical capability and atomic reconciliation route', () => {
    ...
    expect(service).toContain("'number-series'")
  ...
  it('registers the exact confirmation and secure credential policy', () => {
    const resolved = resolveTeachingPublisher('numberSeries')
    expect(resolved).toMatchObject({
      topicSlug:'number-series',
      script:'scripts/create-and-publish-number-series-teaching-system.mjs',
      passwordEnv:'CSE_NUMBER_SERIES_ADMIN_PASSWORD',
      confirmation:'publish-number-series-teaching-system-v1',
      capabilityCheck:true,
    })
    expect(publisher).not.toMatch(/password\s*=\s*['"][^'"]+['"]/u)
  })
  it('keeps the Topic 3 release developer-only while retaining deployed runtime coverage', () => {
    const risk = classifyChangedFiles([
      'scripts/create-and-publish-number-series-teaching-system.mjs',
      'scripts/lib/number-series-teaching-system-content.mjs',
      'tests/number-series-teaching-system.test.ts',
    ])
