...
const email = `agreement-student-${crypto.randomUUID()}@example.test`
...
function agreementTopic(curriculum) { return curriculum.subjects.find((subject) => subject.slug === 'verbal-ability')?.topics.find((topic) => topic.slug === 'subject-verb-agreement') }
...
let topic = agreementTopic(await api('/api/student/courses/cse-professional/curriculum'))
assert(topic?.position === 6 && topic.lessons.length === 12, 'Agreement topic ordering or lesson count is invalid.')
...
topic = agreementTopic(await api('/api/student/courses/cse-professional/curriculum'))
...
for (const question of generated.questions) {
  const expected = /The applicant /iu.test(question.prompt) ? 'submits' : /The applicants /iu.test(question.prompt) ? 'submit' : /She /iu.test(question.prompt) ? 'has' : null
...
console.log('STUDENT_FLOW_OK topic_position=6 lessons=12 intro_blocks=12 generated_questions=5 generated_refresh=stable generated_score=100 fixed_questions=8 fixed_score=100 quiz_questions=15 quiz_score=100 explanations=visible sequential_unlocking=verified')
