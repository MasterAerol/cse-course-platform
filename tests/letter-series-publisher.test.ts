...
import { blocksFor, fixedQuestion, generatedByLesson, lessonSpecs, mixedQuestions, quizQuestions, topicDescription, topicSlug, validateQuestions } from '../scripts/letter-series-topic-content.mjs'
...
describe('Letter Series publisher content', () => {
  it('defines the exact fourth-topic identity and 12 uniquely positioned lessons', () => {
    expect(topicSlug).toBe('letter-series')
    expect(topicDescription).toBe('A structured course on recognizing forward, backward, alternating, gap-based, grouped, and mixed alphabet patterns.')
  ...
  it('provides meaningful mobile-safe teaching blocks before every activity', () => {
    ...
    expect(blocksFor('understanding-alphabet-positions').length).toBeGreaterThanOrEqual(10)
    expect(blocksFor('understanding-alphabet-positions').length).toBeLessThanOrEqual(14)
  })
  ...
  it('reuses an existing topic and rejects duplicates instead of creating another', () => {
    const existing = { id: 9, slug: topicSlug, position: 4 }
  ...
  it('leaves existing Analytical and Numerical Ability generators registered', () => {
    ...
    expect(getGenerator('addition-subtraction-series', 1)).not.toBeNull()
