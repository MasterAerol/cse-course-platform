...
const script = path.join(root, 'scripts', 'create-and-publish-synonyms-antonyms-teaching-system.mjs')
const lessons = synonymsAntonymsLessonSpecs.map((spec, index) => ({ ...spec, id: 101 + index, position: index + 1, status: 'published' as const }))
...
for (const lesson of lessons) {
  ...
  if (lesson.slug === 'positive-neutral-and-negative-tone') {
...
async function handleRequest(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  if (url === '/api/admin/courses/1') {
    send(response, { subjects: [{ slug: 'verbal-ability', topics: [{ slug: 'synonyms-and-antonyms', lessons: lessons.map(({ id, slug, title, lessonType, estimatedMinutes, status, position }) => ({ id, slug, title, lessonType, estimatedMinutes, status, position })) }] }] })
    return
  }

  const reconcileMatch = url.match(/^\/api\/admin\/lessons\/(\d+)\/synonyms-antonyms-teaching-system-v1$/u)
...
function runPublisher(extraArgs: string[] = []): Promise<{ stdout: string; stderr: string; status: number | null }> {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [script, '--base-url', baseUrl, '--cookie', 'test-admin-session', '--confirm', 'publish-synonyms-antonyms-teaching-system-v1', ...extraArgs], { cwd: root, windowsHide: true })
...
describe('Synonyms and Antonyms Teaching System v1 publisher', () => {
  it('validates without mutation, publishes canonically, is idempotent, and fails closed on an unknown deletion', async () => {
    ...
    expect(JSON.parse(validation.stdout)).toMatchObject({
      valid: true,
      topicSlug: 'synonyms-and-antonyms',
      lessonCount: 12,
      writesRequired: true,
    ...
    expect(JSON.parse(second.stdout)).toMatchObject({ published: true, updated: false, topicSlug: 'synonyms-and-antonyms', totals: { lessonsChanged: 0, blocksCreated: 0, blocksUpdated: 0, blocksDeleted: 0, guidedBlocksRemoved: 0 }, unrelatedTopicsModified: 0 })
    ...
    expect(deletionPlan.deletions).toEqual([expect.objectContaining({ lessonSlug: 'understanding-synonyms-and-antonyms', blockType: 'paragraph', identifier: 'Unknown unique legacy learner content.', reason: 'The block is beyond the canonical lesson block sequence.', learnerContentAssessment: 'requires-human-review' })])
