...
import { simpleInterestLessonSpecs } from './lib/simple-interest-teaching-system-content.mjs'

const confirmation = 'publish-simple-interest-teaching-system-v1'
...
function buildPlan(existingBlocks, desiredBlocks, lessonSlug) {
  ...
  const deletions = [...guided, ...excess].map((block) => ({
    ...
    reason: block.type === guidedBlockType ? 'IllustratedGuidedTeaching is outside the approved Simple Interest v1 teaching architecture.' : 'The block is beyond the canonical lesson block sequence.',
...
async function main() {
  ...
  if (cookie === null) {
    ...
    const password = process.env.CSE_SIMPLE_INTEREST_ADMIN_PASSWORD
    if (email === undefined || password === undefined) throw new Error('Pass --cookie, or --email with CSE_SIMPLE_INTEREST_ADMIN_PASSWORD set securely in the environment.')
  ...
  const topic = subject?.topics.find((item) => item.slug === 'simple-interest')
  if (topic === undefined || topic.lessons.length !== simpleInterestLessonSpecs.length) throw new Error('Simple Interest must contain exactly the twelve expected lessons.')

  const states = []
  for (const [lessonIndex, spec] of simpleInterestLessonSpecs.entries()) {
    const lesson = topic.lessons.find((item) => item.slug === spec.slug)
    if (lesson === undefined) throw new Error(`Missing Simple Interest lesson ${spec.slug}.`)
  ...
  if (validateOnly) {
    console.log(JSON.stringify({ valid: true, operation: 'simple-interest-teaching-system-v1', topicSlug: 'simple-interest', lessonCount: simpleInterestLessonSpecs.length, writesRequired: totals.lessonsChanged > 0, totals, deletionReviewRequired, deletionPlanFingerprint, deletions: deletionPlan, lessons: reports }, null, 2))
    return
  }
  ...
  for (const state of states) {
    ...
    await request(`/api/admin/lessons/${state.lesson.id}/simple-interest-teaching-system-v1`, {
      method: 'PUT',
  ...
  console.log(JSON.stringify({ published: true, updated: totals.lessonsChanged > 0, operation: 'simple-interest-teaching-system-v1', topicSlug: 'simple-interest', lessonCount: simpleInterestLessonSpecs.length, totals, lessons: verifiedLessons, unrelatedTopicsModified: 0 }, null, 2))
}
