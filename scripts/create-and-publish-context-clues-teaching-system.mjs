...
import { contextCluesLessonSpecs } from './lib/context-clues-teaching-system-content.mjs'

const confirmation = 'publish-context-clues-teaching-system-v1'
...
function buildPlan(existingBlocks, desiredBlocks, lessonSlug) {
  ...
  const deletions = [...guided, ...excess].map((block) => ({
    ...
    reason: block.type === guidedBlockType ? 'IllustratedGuidedTeaching is outside the approved Context Clues v1 teaching architecture.' : 'The block is beyond the canonical lesson block sequence.',
...
async function main() {
  ...
  if (cookie === null) {
    ...
    const password = process.env.CSE_CONTEXT_CLUES_ADMIN_PASSWORD
    if (email === undefined || password === undefined) throw new Error('Pass --cookie, or --email with CSE_CONTEXT_CLUES_ADMIN_PASSWORD set securely in the environment.')
  ...
  const topic = subject?.topics.find((item) => item.slug === 'context-clues')
  if (topic === undefined || topic.lessons.length !== contextCluesLessonSpecs.length) throw new Error('Context Clues must contain exactly the twelve expected lessons.')

  const states = []
  for (const [lessonIndex, spec] of contextCluesLessonSpecs.entries()) {
    const lesson = topic.lessons.find((item) => item.slug === spec.slug)
    if (lesson === undefined) throw new Error(`Missing Context Clues lesson ${spec.slug}.`)
  ...
  if (validateOnly) {
    console.log(JSON.stringify({ valid: true, operation: 'context-clues-teaching-system-v1', topicSlug: 'context-clues', lessonCount: contextCluesLessonSpecs.length, writesRequired: totals.lessonsChanged > 0, totals, deletionReviewRequired, deletionPlanFingerprint, deletions: deletionPlan, lessons: reports }, null, 2))
    return
  }
  ...
  for (const state of states) {
    ...
    await request(`/api/admin/lessons/${state.lesson.id}/context-clues-teaching-system-v1`, {
      method: 'PUT',
  ...
  console.log(JSON.stringify({ published: true, updated: totals.lessonsChanged > 0, operation: 'context-clues-teaching-system-v1', topicSlug: 'context-clues', lessonCount: contextCluesLessonSpecs.length, totals, lessons: verifiedLessons, unrelatedTopicsModified: 0 }, null, 2))
}
