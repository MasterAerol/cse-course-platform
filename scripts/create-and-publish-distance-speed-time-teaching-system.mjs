...
import { distanceSpeedTimeLessonSpecs } from './lib/distance-speed-time-teaching-system-content.mjs'

const confirmation = 'publish-distance-speed-time-teaching-system-v1'
...
function buildPlan(existingBlocks, desiredBlocks, lessonSlug) {
  ...
  const deletions = [...guided, ...excess].map((block) => ({
    ...
    reason: block.type === guidedBlockType ? 'IllustratedGuidedTeaching is outside the approved Distance, Speed and Time v1 teaching architecture.' : 'The block is beyond the canonical lesson block sequence.',
...
async function main() {
  ...
  if (cookie === null) {
    ...
    const password = process.env.CSE_DISTANCE_SPEED_TIME_ADMIN_PASSWORD
    if (email === undefined || password === undefined) throw new Error('Pass --cookie, or --email with CSE_DISTANCE_SPEED_TIME_ADMIN_PASSWORD set securely in the environment.')
  ...
  const topic = subject?.topics.find((item) => item.slug === 'distance-speed-and-time')
  if (topic === undefined || topic.lessons.length !== distanceSpeedTimeLessonSpecs.length) throw new Error('Distance, Speed and Time must contain exactly the twelve expected lessons.')

  const states = []
  for (const [lessonIndex, spec] of distanceSpeedTimeLessonSpecs.entries()) {
    const lesson = topic.lessons.find((item) => item.slug === spec.slug)
    if (lesson === undefined) throw new Error(`Missing Distance, Speed and Time lesson ${spec.slug}.`)
  ...
  if (validateOnly) {
    console.log(JSON.stringify({ valid: true, operation: 'distance-speed-time-teaching-system-v1', topicSlug: 'distance-speed-and-time', lessonCount: distanceSpeedTimeLessonSpecs.length, writesRequired: totals.lessonsChanged > 0, totals, deletionReviewRequired, deletionPlanFingerprint, deletions: deletionPlan, lessons: reports }, null, 2))
    return
  }
  ...
  for (const state of states) {
    ...
    await request(`/api/admin/lessons/${state.lesson.id}/distance-speed-time-teaching-system-v1`, {
      method: 'PUT',
  ...
  console.log(JSON.stringify({ published: true, updated: totals.lessonsChanged > 0, operation: 'distance-speed-time-teaching-system-v1', topicSlug: 'distance-speed-and-time', lessonCount: distanceSpeedTimeLessonSpecs.length, totals, lessons: verifiedLessons, unrelatedTopicsModified: 0 }, null, 2))
}
