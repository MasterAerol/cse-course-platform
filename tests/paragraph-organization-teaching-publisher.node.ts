...
const script=path.join(root,'scripts','create-and-publish-paragraph-organization-teaching-system.mjs')
const lessons=paragraphOrganizationLessonSpecs.map((spec,index)=>({...spec,id:301+index,position:index+1,status:'published' as const}))
const blocksByLesson=new Map<number,StoredBlock[]>()
let nextBlockId=3_000
let mutationCalls=0
...
for(const [lessonIndex,item] of lessons.entries()){
  const blocks=Array.from({length:productionCounts[lessonIndex]??0},(_,index):StoredBlock=>({id:nextBlockId++,type:'paragraph',content:{text:`Current production ${item.slug} block ${index+1}`},position:index+1}))
  if(item.slug==='comparison-contrast-order') preservedRetainedBlockId=blocks[4]?.id??0
  blocksByLesson.set(item.id,blocks)
}
...
async function handle(request:IncomingMessage,response:ServerResponse){
  ...
  if(url==='/api/admin/courses/1'){send(response,{subjects:[{slug:'verbal-ability',topics:[{slug:'paragraph-organization',lessons:lessons.map(({id,slug,title,lessonType,estimatedMinutes,status,position})=>({id,slug,title,lessonType,estimatedMinutes,status,position}))}]}]});return}
  const capability=url.match(/^\/api\/admin\/lessons\/(\d+)\/paragraph-organization-teaching-system-v1\/capability$/u)
  if(capability?.[1]!==undefined&&request.method==='GET'){capabilityCalls+=1;send(response,{supported:true,operation:'paragraph-organization-teaching-system-v1',topicSlug:'paragraph-organization'});return}
  const reconcile=url.match(/^\/api\/admin\/lessons\/(\d+)\/paragraph-organization-teaching-system-v1$/u)
...
function runPublisher(extraArgs:string[]=[]):Promise<{stdout:string;stderr:string;status:number|null}>{return new Promise((resolve,reject)=>{const child=spawn(process.execPath,[script,'--base-url',baseUrl,'--cookie','test-admin-session','--confirm','publish-paragraph-organization-teaching-system-v1',...extraArgs],{cwd:root,windowsHide:true});let stdout='';let stderr='';child.stdout.on('data',(chunk)=>{stdout+=String(chunk)});child.stderr.on('data',(chunk)=>{stderr+=String(chunk)});child.on('error',reject);child.on('close',(status)=>resolve({stdout,stderr,status}))})}

describe('Paragraph Organization Teaching System v1 publisher',()=>{
  it('validates read-only, publishes canonically, preserves IDs, proves idempotency, and fails closed on unknown deletion',async()=>{
    const validation=await runPublisher(['--validate-only']);expect(validation).toMatchObject({status:0,stderr:''});expect(JSON.parse(validation.stdout)).toMatchObject({valid:true,topicSlug:'paragraph-organization',lessonCount:12,writesRequired:true,deletionReviewRequired:false,deletionPlanFingerprint:null,totals:{lessonsChanged:12,blocksCreated:desiredBlockCount-productionBlockCount,blocksUpdated:productionBlockCount,blocksDeleted:0,guidedBlocksRemoved:0}});expect(mutationCalls).toBe(0);expect(capabilityCalls).toBeGreaterThan(0)
    const first=await runPublisher();expect(first).toMatchObject({status:0,stderr:''});expect(JSON.parse(first.stdout)).toMatchObject({published:true,updated:true,topicSlug:'paragraph-organization',lessonCount:12,totals:{lessonsChanged:12,blocksCreated:desiredBlockCount-productionBlockCount,blocksUpdated:productionBlockCount,blocksDeleted:0,guidedBlocksRemoved:0},unrelatedTopicsModified:0});expect(findBlock(preservedRetainedBlockId)).not.toBeNull()
    ...
    const second=await runPublisher();expect(second).toMatchObject({status:0,stderr:''});expect(JSON.parse(second.stdout)).toMatchObject({published:true,updated:false,topicSlug:'paragraph-organization',totals:{lessonsChanged:0,blocksCreated:0,blocksUpdated:0,blocksDeleted:0,guidedBlocksRemoved:0},unrelatedTopicsModified:0});expect(mutationCalls).toBe(mutationsAfterFirst)
