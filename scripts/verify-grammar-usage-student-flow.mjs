...
const email = `grammar-student-${crypto.randomUUID()}@example.test`
...
function grammarTopic(curriculum) { return curriculum.subjects.find((subject) => subject.slug === 'verbal-ability')?.topics.find((topic) => topic.slug === 'grammar-and-correct-usage') }
...
let topic = grammarTopic(await api('/api/student/courses/cse-professional/curriculum'))
assert(topic?.position === 5 && topic.lessons.length === 12, 'Grammar topic ordering or lesson count is invalid.')
...
topic = grammarTopic(await api('/api/student/courses/cse-professional/curriculum'))
...
for (const question of generated.questions) {
  const expected = /employee responded/iu.test(question.prompt) ? 'promptly' : /committee reached/iu.test(question.prompt) ? 'decision' : /files yesterday/iu.test(question.prompt) ? 'organized' : null
...
console.log('STUDENT_FLOW_OK topic_position=5 lessons=12 intro_blocks=12 generated_questions=5 generated_refresh=stable generated_score=100 fixed_questions=8 fixed_score=100 quiz_questions=15 quiz_score=100 explanations=visible sequential_unlocking=verified')
