...
const confirmation = 'publish-ratio-proportion-teaching-system-v1'
...
function buildPlan(existingBlocks, desiredBlocks) {
  ...
  const deletions = [...guided, ...excess].map((block) => ({
    ...
    reason: block.type === guidedBlockType ? 'IllustratedGuidedTeaching is outside the approved Ratio and Proportion v1 teaching architecture.' : 'The block is beyond the canonical lesson block sequence.',
...
async function main() {
  ...
  if (cookie === null) {
    ...
    const password = args.get('password') ?? process.env.CSE_RATIO_PROPORTION_ADMIN_PASSWORD
    if (email === undefined || password === undefined) throw new Error('Pass --cookie, or --email with --password or CSE_RATIO_PROPORTION_ADMIN_PASSWORD.')
  ...
  const topic = subject?.topics.find((item) => item.slug === 'ratio-and-proportion')
  if (topic === undefined || topic.lessons.length !== ratioProportionLessonSpecs.length) throw new Error('Ratio and Proportion must contain exactly the twelve expected lessons.')

  const states = []
  for (const [lessonIndex, spec] of ratioProportionLessonSpecs.entries()) {
    const lesson = topic.lessons.find((item) => item.slug === spec.slug)
    if (lesson === undefined) throw new Error(`Missing Ratio and Proportion lesson ${spec.slug}.`)
  ...
  if (validateOnly) {
    console.log(JSON.stringify({ valid: true, operation: 'ratio-proportion-teaching-system-v1', topicSlug: 'ratio-and-proportion', lessonCount: ratioProportionLessonSpecs.length, writesRequired: totals.lessonsChanged > 0, totals, deletionReviewRequired, deletionPlanFingerprint, deletions: deletionPlan, lessons: reports }, null, 2))
    return
  }
  ...
  for (const state of states) {
    ...
    await request(`/api/admin/lessons/${state.lesson.id}/ratio-proportion-teaching-system-v1`, {
      method: 'PUT',
  ...
  console.log(JSON.stringify({ published: true, updated: totals.lessonsChanged > 0, operation: 'ratio-proportion-teaching-system-v1', topicSlug: 'ratio-and-proportion', lessonCount: ratioProportionLessonSpecs.length, totals, lessons: verifiedLessons, unrelatedTopicsModified: 0 }, null, 2))
}
