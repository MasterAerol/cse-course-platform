...
const email = `ra-6713-student-${crypto.randomUUID()}@example.test`
...
function ra6713Topic(curriculum) { return curriculum.subjects.find((subject) => subject.slug === 'general-information')?.topics.find((topic) => topic.slug === 'ra-6713-code-of-conduct') }

await api('/api/auth/register', { method: 'POST', body: JSON.stringify({ email, password, firstName: 'RA6713', lastName: 'Student' }) })
localSql(`INSERT INTO course_enrollments (user_id,course_id,enrollment_status,enrollment_source) SELECT u.id,c.id,'active','admin' FROM users u CROSS JOIN courses c WHERE u.email='${email}' AND c.slug='cse-professional'; INSERT INTO lesson_progress (user_id,lesson_id,status,started_at,completed_at,last_viewed_at,progress_percent) SELECT u.id,l.id,'completed',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,100 FROM users u CROSS JOIN lessons l JOIN topics t ON t.id=l.topic_id JOIN subjects s ON s.id=t.subject_id WHERE u.email='${email}' AND (s.position<4 OR t.slug='philippine-constitution-fundamentals');`)

let topic = ra6713Topic(await api('/api/student/courses/cse-professional/curriculum'))
assert(topic?.position === 2 && topic.lessons.length === 12, 'RA 6713 topic ordering or lesson count is invalid.')
...
topic = ra6713Topic(await api('/api/student/courses/cse-professional/curriculum'))
...
assert(generated.questions.every((question) => question.choices.length === 4), 'Generated RA 6713 choices are invalid.')
...
assert(generatedResult.questions.every((question) => /(Section|Rule)/u.test(JSON.stringify(question.explanation))), 'Generated review lacks legal source references.')

localSql(`UPDATE lesson_progress SET status='completed',completed_at=CURRENT_TIMESTAMP,last_viewed_at=CURRENT_TIMESTAMP,progress_percent=100 WHERE user_id=(SELECT id FROM users WHERE email='${email}') AND lesson_id IN (SELECT l.id FROM lessons l JOIN topics t ON t.id=l.topic_id WHERE t.slug='ra-6713-code-of-conduct' AND l.position<=10); INSERT OR IGNORE INTO lesson_progress (user_id,lesson_id,status,started_at,completed_at,last_viewed_at,progress_percent) SELECT u.id,l.id,'completed',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,100 FROM users u CROSS JOIN lessons l JOIN topics t ON t.id=l.topic_id WHERE u.email='${email}' AND t.slug='ra-6713-code-of-conduct' AND l.position<=10;`)
topic = ra6713Topic(await api('/api/student/courses/cse-professional/curriculum'))
...
assert(fixedResult.scorePercent === 100 && fixedResult.passed && fixedResult.questions.every((question) => /(Section|Rule)/u.test(JSON.stringify(question.explanation))), 'Fixed scoring or legal-source review failed.')
...
assert(quizResult.scorePercent === 100 && quizResult.passed && quizResult.questions.every((question) => /(Section|Rule)/u.test(JSON.stringify(question.explanation))), 'Quiz scoring or legal-source review failed.')
console.log('STUDENT_FLOW_OK topic_position=2 lessons=12 intro_blocks=13 generated_questions=5 source_metadata=preserved generated_refresh=stable generated_score=100 fixed_questions=12 fixed_score=100 quiz_questions=20 quiz_score=100 explanations=visible sequential_unlocking=verified')
