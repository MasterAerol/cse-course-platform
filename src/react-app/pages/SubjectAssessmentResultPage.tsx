...
export function SubjectAssessmentResultPage() {
  ...
  return (
    <main className="dashboard-page">
      ...
      <section className="dashboard-card assessment-result">
        <p className="eyebrow">Attempt {result.attempt.attemptNumber}</p>
        <h1>{result.passed ? 'Assessment passed' : 'Keep building your skills'}</h1>
        <p className="assessment-score">
          {result.earnedPoints}/{result.totalPoints} · {result.scorePercent}%
        </p>
        <p>{result.feedback}</p>
        <p>
          Strongest: <strong>{result.breakdown.strongestTopic.topicTitle}</strong> · Weakest:{' '}
          <strong>{result.breakdown.weakestTopic.topicTitle}</strong>
        </p>
        <div className="topic-breakdown">
          {result.breakdown.topics.map((topic) => (
            <article key={topic.topicSlug}>
              <strong>{topic.topicTitle}</strong>
              <span>
                {topic.correctCount}/{topic.totalQuestions} · {topic.status}
              </span>
            </article>
          ))}
        </div>
