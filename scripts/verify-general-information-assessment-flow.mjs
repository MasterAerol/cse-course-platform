...
async function register(label) {
  const email = `general-information-assessment-${label}-${crypto.randomUUID()}@example.test`
  const password = `Aa1!${crypto.randomUUID()}`
  cookie = null
  await api('/api/auth/register', { method: 'POST', body: JSON.stringify({ email, password, firstName: 'General', lastName: label }) })
  enroll(email)
  return { email, cookie }
}

const owner = await register('Owner')
let summary = await api('/api/student/subject-assessments/general-information-subject-assessment')
assert(summary.assessment.title === 'General Information Subject Assessment' && summary.assessment.questionCount === 40 && summary.assessment.passingScore === 70, 'Assessment identity is invalid.')
...
const started = await api('/api/student/subject-assessments/general-information-subject-assessment/attempts', { method: 'POST' })
assert(started.questions.length === 40 && started.answeredCount === 0 && started.totalCount === 40, 'Attempt did not create exactly 40 snapshots.')
...
assert(correctRows.length === 40, 'Stored attempt does not have 40 correct-choice snapshots.')
...
assert(result.earnedPoints === 40 && result.totalPoints === 40 && result.scorePercent === 100 && result.passed, 'Server-side all-correct scoring failed.')
assert(result.breakdown.topics.length === 4 && result.breakdown.topics.every((topic) => topic.totalQuestions === 10), 'Four-topic result distribution is invalid.')
...
assert(review.questions.length === 40 && review.questions.every((question) => question.explanation?.length > 0 && question.correctChoice !== undefined && /(Article|Section|RA |PD |UDHR|UN |OHCHR|ICCPR|ICESCR|Mandate|Constitution)/u.test(question.explanation)), 'Submitted review or source references are incomplete.')
summary = await api('/api/student/subject-assessments/general-information-subject-assessment')
...
const retry = await api('/api/student/subject-assessments/general-information-subject-assessment/attempts', { method: 'POST' })
assert(retry.attempt.publicId !== started.attempt.publicId && retry.attempt.attemptNumber === 2 && retry.questions.length === 40, 'Retake did not create a new 40-question attempt.')
const audit = d1(`SELECT (SELECT COUNT(*) FROM subject_assessment_question_snapshots s JOIN subject_assessment_attempts a ON a.id=s.attempt_id WHERE a.public_id='${started.attempt.publicId}') AS snapshots, (SELECT COUNT(*) FROM (SELECT s.topic_slug,COUNT(*) count FROM subject_assessment_question_snapshots s JOIN subject_assessment_attempts a ON a.id=s.attempt_id WHERE a.public_id='${started.attempt.publicId}' GROUP BY s.topic_slug HAVING count<>10)) AS bad_topics, (SELECT COUNT(*) FROM (SELECT s.topic_slug,s.difficulty,COUNT(*) count FROM subject_assessment_question_snapshots s JOIN subject_assessment_attempts a ON a.id=s.attempt_id WHERE a.public_id='${started.attempt.publicId}' GROUP BY s.topic_slug,s.difficulty HAVING count<>CASE s.difficulty WHEN 'hard' THEN 2 ELSE 4 END)) AS bad_difficulty;`, true)[0]
assert(audit.snapshots === 40 && audit.bad_topics === 0 && audit.bad_difficulty === 0, 'Persisted snapshot allocation audit failed.')
console.log(`GENERAL_INFORMATION_ASSESSMENT_FLOW_OK questions=40 topics=4 difficulty=4/4/2 refresh=stable saved=replaceable ownership=protected pre_submit=hidden score=40/40 review=40 sources=visible history=verified retry=verified other=${other.email}`)