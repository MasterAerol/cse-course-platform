...
import publisher from '../scripts/create-and-publish-data-interpretation-teaching-system.mjs?raw'

describe('Data Interpretation Teaching Release capability', () => {
  it('uses the deployed shared Analytical capability and atomic reconciliation route', () => {
    ...
    expect(service).toContain("'data-interpretation'")
  ...
  it('registers the exact confirmation and secure credential policy', () => {
    const resolved = resolveTeachingPublisher('dataInterpretation')
    expect(resolved).toMatchObject({
      topicSlug:'data-interpretation',
      script:'scripts/create-and-publish-data-interpretation-teaching-system.mjs',
      passwordEnv:'CSE_DATA_INTERPRETATION_ADMIN_PASSWORD',
  ...
  it('keeps the Topic 9 release developer-only while retaining deployed runtime coverage', () => {
    const risk = classifyChangedFiles([
      'scripts/create-and-publish-data-interpretation-teaching-system.mjs',
      'scripts/lib/data-interpretation-teaching-system-content.mjs',
      'tests/data-interpretation-teaching-system.test.ts',
    ])
