...
const email = `paragraph-organization-student-${crypto.randomUUID()}@example.test`
...
function paragraphTopic(curriculum) { return curriculum.subjects.find((subject) => subject.slug === 'verbal-ability')?.topics.find((topic) => topic.slug === 'paragraph-organization') }

await api('/api/auth/register', { method: 'POST', body: JSON.stringify({ email, password, firstName: 'Paragraph', lastName: 'Student' }) })
...
let topic = paragraphTopic(await api('/api/student/courses/cse-professional/curriculum'))
assert(topic?.position === 9 && topic.lessons.length === 12, 'Paragraph Organization topic ordering or lesson count is invalid.')
...
topic = paragraphTopic(await api('/api/student/courses/cse-professional/curriculum'))
...
for (const question of generated.questions) {
  const expected = /service counter follows/iu.test(question.prompt) ? 'The service counter follows a clear morning routine.' : /accurate records support/iu.test(question.prompt) ? 'Accurate records support efficient office work.' : /community clean-up succeeds/iu.test(question.prompt) ? 'A community clean-up succeeds through organized teamwork.' : null
...
console.log('STUDENT_FLOW_OK topic_position=9 lessons=12 intro_blocks=12 generated_questions=5 generated_refresh=stable generated_score=100 fixed_questions=8 fixed_score=100 quiz_questions=15 quiz_score=100 explanations=visible sequential_unlocking=verified')
