...
export async function getParagraphOrganizationTeachingSystemCapability(
  database: D1Database,
  lessonId: number,
): Promise<{ supported: true; operation: string; topicSlug: string }> {
  const lesson = await findLessonById(database, lessonId)
  ...
  const course = subject === null ? null : await findCourseById(database, subject.course_id)
  if (
    topic?.slug !== 'paragraph-organization' ||
    subject?.slug !== 'verbal-ability' ||
    course?.slug !== 'cse-professional'
  ) {
    throw new AppError(
      409,
      'PARAGRAPH_ORGANIZATION_TEACHING_SYSTEM_TARGET_MISMATCH',
      'Paragraph Organization Teaching System capability is restricted to the CSE Verbal Ability topic.',
    )
  }
  return { supported: true, operation: 'paragraph-organization-teaching-system-v1', topicSlug: topic.slug }
}
export async function reconcileParagraphOrganizationTeachingSystemLesson(
  database: D1Database,
  actor: AuthenticatedPrincipal,
  lessonId: number,
  input: ParagraphOrganizationTeachingSystemReconcileInput,
): Promise<{
  blocks: AdminLessonBlock[]
  writeRequired: boolean
  createdCount: number
  ...
  if (
    ...
    throw new AppError(
      409,
      'PARAGRAPH_ORGANIZATION_TEACHING_SYSTEM_TARGET_MISMATCH',
      'Paragraph Organization Teaching System reconciliation is restricted to the CSE Verbal Ability topic.',
    )
  }
  ...
  if (desired.some((block) => block.blockType === 'illustrated-guided-teaching')) {
    throw new AppError(
      409,
      'PARAGRAPH_ORGANIZATION_GUIDED_TEACHING_NOT_ALLOWED',
      'Paragraph Organization Teaching System v1 does not include illustrated guided teaching.',
    )
  }
  ...
  const retained = allowed.slice(0, retainedCount).map((block, index) => {
    const target = desired[index]
    if (target === undefined) throw new Error('Paragraph Organization reconciliation target is missing.')
    return {
      id: block.id,
      blockType: target.blockType,
      contentJson: target.contentJson,
  ...
  if (writeRequired) {
    await reconcileTeachingSystemLessonBlocksWithAudit(database, {
      ...
      metadataJson: JSON.stringify({
        lessonId,
        operation: 'paragraph-organization-teaching-system-v1-reconcile',
        createdCount: creates.length,
        updatedCount,
        deletedCount: deleteIds.length,
      }),
...
export async function getReadingComprehensionTeachingSystemCapability(
  database: D1Database,
  lessonId: number,
): Promise<{ supported: true; operation: string; topicSlug: string }> {
  const lesson = await findLessonById(database, lessonId)
  ...
  const course = subject === null ? null : await findCourseById(database, subject.course_id)
  if (
    topic?.slug !== 'reading-comprehension' ||
    subject?.slug !== 'verbal-ability' ||
    course?.slug !== 'cse-professional'
  ) {
    throw new AppError(
      409,
      'READING_COMPREHENSION_TEACHING_SYSTEM_TARGET_MISMATCH',
      'Reading Comprehension Teaching System capability is restricted to the CSE Verbal Ability topic.',
    )
  }
  return { supported: true, operation: 'reading-comprehension-teaching-system-v1', topicSlug: topic.slug }
}
export async function reconcileReadingComprehensionTeachingSystemLesson(
  database: D1Database,
  actor: AuthenticatedPrincipal,
  lessonId: number,
  input: ReadingComprehensionTeachingSystemReconcileInput,
): Promise<{
  blocks: AdminLessonBlock[]
  writeRequired: boolean
  createdCount: number
  ...
  if (
    ...
    throw new AppError(
      409,
      'READING_COMPREHENSION_TEACHING_SYSTEM_TARGET_MISMATCH',
      'Reading Comprehension Teaching System reconciliation is restricted to the CSE Verbal Ability topic.',
    )
  }
  ...
  if (desired.some((block) => block.blockType === 'illustrated-guided-teaching')) {
    throw new AppError(
      409,
      'READING_COMPREHENSION_GUIDED_TEACHING_NOT_ALLOWED',
      'Reading Comprehension Teaching System v1 does not include illustrated guided teaching.',
    )
  }
  ...
  const retained = allowed.slice(0, retainedCount).map((block, index) => {
    const target = desired[index]
    if (target === undefined) throw new Error('Reading Comprehension reconciliation target is missing.')
    return {
      id: block.id,
      blockType: target.blockType,
      contentJson: target.contentJson,
  ...
  if (writeRequired) {
    await reconcileTeachingSystemLessonBlocksWithAudit(database, {
      ...
      metadataJson: JSON.stringify({
        lessonId,
        operation: 'reading-comprehension-teaching-system-v1-reconcile',
        createdCount: creates.length,
        updatedCount,
        deletedCount: deleteIds.length,
      }),
...
const analyticalTeachingSystemTopicSlugs = new Set([
  ...
  'syllogisms',
  'seating-and-arrangement-problems',
  'data-interpretation',
])

export async function getAnalyticalTeachingSystemCapability(
  database: D1Database,
  lessonId: number,
): Promise<{ supported: true; operation: string; topicSlug: string }> {
  const lesson = await findLessonById(database, lessonId)
  ...
  if (
    topic === null ||
    !analyticalTeachingSystemTopicSlugs.has(topic.slug) ||
    subject?.slug !== 'analytical-ability' ||
    course?.slug !== 'cse-professional'
  ) {
    throw new AppError(
      409,
      'ANALYTICAL_TEACHING_SYSTEM_TARGET_MISMATCH',
      'Analytical Teaching System capability is restricted to authoritative CSE Analytical Ability topics.',
    )
  }
  return { supported: true, operation: topic.slug + '-teaching-system-v1', topicSlug: topic.slug }
}

export async function reconcileAnalyticalTeachingSystemLesson(
  database: D1Database,
  actor: AuthenticatedPrincipal,
  lessonId: number,
  input: AnalyticalTeachingSystemReconcileInput,
): Promise<{
  blocks: AdminLessonBlock[]
  writeRequired: boolean
  createdCount: number
  ...
  if (
    ...
    throw new AppError(
      409,
      'ANALYTICAL_TEACHING_SYSTEM_TARGET_MISMATCH',
      'Analytical Teaching System reconciliation is restricted to authoritative CSE Analytical Ability topics.',
    )
  }
  ...
  if (desired.some((block) => block.blockType === 'illustrated-guided-teaching')) {
    throw new AppError(
      409,
      'ANALYTICAL_GUIDED_TEACHING_NOT_ALLOWED',
      'Analytical Teaching System v1 uses text-first VisualTeachingBoard examples rather than illustrated guided teaching.',
    )
  }
  ...
  const retained = allowed.slice(0, retainedCount).map((block, index) => {
    const target = desired[index]
    if (target === undefined) throw new Error('Analytical Teaching System reconciliation target is missing.')
    return {
      id: block.id,
      blockType: target.blockType,
      contentJson: target.contentJson,
