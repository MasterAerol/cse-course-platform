...
async function main() {
  ...
  if (cookie === null) {
    ...
    const password = process.env.CSE_AVERAGE_ADMIN_PASSWORD
    if (email === undefined || password === undefined) throw new Error('Pass --cookie, or --email with CSE_AVERAGE_ADMIN_PASSWORD set securely in the environment.')
  ...
  const topic = subject?.topics.find((item) => item.slug === 'average')
  if (topic === undefined || topic.lessons.length !== averageLessonSpecs.length) throw new Error('Average must contain exactly the twelve expected lessons.')

  const states = []
  for (const [lessonIndex, spec] of averageLessonSpecs.entries()) {
    const lesson = topic.lessons.find((item) => item.slug === spec.slug)
    if (lesson === undefined) throw new Error(`Missing Average lesson ${spec.slug}.`)
  ...
  if (validateOnly) {
    console.log(JSON.stringify({ valid: true, operation: 'average-teaching-system-v1', topicSlug: 'average', lessonCount: averageLessonSpecs.length, writesRequired: totals.lessonsChanged > 0, totals, deletionReviewRequired, deletionPlanFingerprint, deletions: deletionPlan, lessons: reports }, null, 2))
    return
  }
  ...
  for (const state of states) {
    ...
    await request(`/api/admin/lessons/${state.lesson.id}/average-teaching-system-v1`, {
      method: 'PUT',
  ...
  console.log(JSON.stringify({ published: true, updated: totals.lessonsChanged > 0, operation: 'average-teaching-system-v1', topicSlug: 'average', lessonCount: averageLessonSpecs.length, totals, lessons: verifiedLessons, unrelatedTopicsModified: 0 }, null, 2))
}
