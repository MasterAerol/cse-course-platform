...
const email = `sentence-structure-student-${crypto.randomUUID()}@example.test`
...
function sentenceStructureTopic(curriculum) { return curriculum.subjects.find((subject) => subject.slug === 'verbal-ability')?.topics.find((topic) => topic.slug === 'sentence-structure-and-error-identification') }

await api('/api/auth/register', { method: 'POST', body: JSON.stringify({ email, password, firstName: 'Structure', lastName: 'Student' }) })
...
let topic = sentenceStructureTopic(await api('/api/student/courses/cse-professional/curriculum'))
assert(topic?.position === 8 && topic.lessons.length === 12, 'Sentence Structure topic ordering or lesson count is invalid.')
...
topic = sentenceStructureTopic(await api('/api/student/courses/cse-professional/curriculum'))
...
for (const question of generated.questions) {
  const expected = /new records clerk/iu.test(question.prompt) ? 'The new records clerk' : /review committee/iu.test(question.prompt) ? 'will announce the schedule tomorrow' : /records arrived late/iu.test(question.prompt) ? 'Because the records arrived late' : null
...
console.log('STUDENT_FLOW_OK topic_position=8 lessons=12 intro_blocks=12 generated_questions=5 generated_refresh=stable generated_score=100 fixed_questions=8 fixed_score=100 quiz_questions=15 quiz_score=100 explanations=visible sequential_unlocking=verified')
