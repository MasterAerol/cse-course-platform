...
import { blocksFor, fixedQuestion, generatedByLesson, lessonSpecs, mixedQuestions, quizQuestions, subjectSlug, topicDescription, topicSlug, validateQuestions } from '../scripts/sentence-completion-topic-content.mjs'
...
describe('Sentence Completion publisher content', () => {
  it('defines the fourth Verbal topic and exactly twelve ordered lessons', () => {
    expect(subjectSlug).toBe('verbal-ability')
    expect(topicSlug).toBe('sentence-completion')
    expect(topicDescription).toBe('A structured course on completing sentences using grammar, meaning, logic, transitions, tone, parallel structure, and contextual relationships.')
  ...
  it('provides mobile-safe instructional blocks and nine registered practices', () => {
    ...
    expect(blocksFor('understanding-sentence-completion')).toHaveLength(12)
